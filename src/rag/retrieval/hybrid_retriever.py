...
class HybridRetriever(BaseRetriever, BaseModel):
    ...
    vector_retriever: VectorRetriever = Field(description="Dense vector retriever component")
    bm25_retriever: BM25Retriever = Field(description="Sparse BM25 retriever component")
    alpha: float = Field(default=0.5, description="Weight for vector scores (1-alpha will be BM25 weight)")
    k: int = Field(default=5, description="Number of final results to return")
    fetch_k: Optional[int] = Field(default=None, description="Number of results to fetch from each retriever")
    ...
    def __init__(
        ...
        # Initialize both parent classes with a single call to super().__init__()
        super().__init__(
            vector_retriever=vector_retriever,
            bm25_retriever=bm25_retriever,
            alpha=alpha,
            k=k,
            fetch_k=fetch_k or 2 * k
        )
    ...
    def _get_relevant_documents(self, query: str) -> List[Document]:
        ...
        """
        # Get results from both retrievers using methods that return (Document, score) pairs
        vector_results = self.vector_retriever.retrieve(query, k=self.fetch_k)
        bm25_results = self.bm25_retriever.retrieve_with_scores(query, k=self.fetch_k)
